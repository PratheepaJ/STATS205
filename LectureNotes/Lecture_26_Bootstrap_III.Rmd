...
- $\hat{a} = \dfrac{\sigma_{i=1}^{n}\left(\hat{\theta}_{(\cdot)} -\hat{\theta}_{(i)}  \right)^{3}}{6 \left\lbrace \sigma_{i=1}^{n}\left(\hat{\theta}_{(\cdot)} -\hat{\theta}_{(i)}  \right)^{2} \right\rbrace^{3/2}}$.
